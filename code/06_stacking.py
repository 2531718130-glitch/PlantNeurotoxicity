"""
06_stacking.py
Train a Stacking ensemble (RF + XGBoost + SVM + KNN, meta-learner = Logistic Regression).
...
Notes:
  ...
  - Naive Bayes is intentionally excluded because its probability calibration
    is unreliable on this mixed feature space.

Input:  data/step3_final_v4.csv
Output: OOF probabilities and test-set predictions from the Stacking ensemble.
"""
...
print("=" * 70)
print("Step 5: Stacking ensemble training with 5-fold cross-validation")
print("=" * 70)
...
def build_base_learners(scale_pos_weight=1.0):
    """Return fresh instances of the four base learners used in Stacking."""
...
print("\n" + "=" * 70)
print("Stacking 5-fold cross-validation (in-fold standardization, OOF collected)")
print("=" * 70)

skf = StratifiedKFold(n_splits=config.N_FOLDS, shuffle=True, random_state=config.RANDOM_STATE)

model_names = ['KNN', 'Random Forest', 'XGBoost', 'SVM', 'Stacking']
...
for fold, (train_idx, val_idx) in enumerate(skf.split(X, y)):
    ...
    meta = LogisticRegression(max_iter=1000, random_state=config.RANDOM_STATE)
...
for name, probs in oof_probs.items():
    ...
    np.save(os.path.join(config.RESULTS_DIR, f'stacking_prob_{key}_oof.npy'), probs)
np.save(os.path.join(config.RESULTS_DIR, 'stacking_label_oof.npy'), oof_labels)
print(f"\nOOF probabilities saved to: {config.RESULTS_DIR}")

# ==================== Final model on fixed split ====================
print("\n" + "=" * 70)
print("Final Stacking model trained on fixed train/val/test split")
print("=" * 70)
...
meta_f = LogisticRegression(max_iter=1000, random_state=config.RANDOM_STATE)

stacking_f = StackingClassifier(
...
stacking_f.fit(X_train_s, y_train)

val_auc = roc_auc_score(y_val, stacking_f.predict_proba(X_val_s)[:, 1])
test_auc = roc_auc_score(y_test, stacking_f.predict_proba(X_test_s)[:, 1])
print(f"\nStacking validation AUC: {val_auc:.4f}")
print(f"Stacking test AUC: {test_auc:.4f}")

# ==================== Save ====================
with open(config.MODEL_STACKING, 'wb') as f:
    pickle.dump({'model': stacking_f, 'scaler': scaler}, f)

np.save(os.path.join(config.RESULTS_DIR, 'stacking_prob_test.npy'),
        stacking_f.predict_proba(X_test_s)[:, 1])
np.save(os.path.join(config.RESULTS_DIR, 'stacking_label_test.npy'), y_test)
